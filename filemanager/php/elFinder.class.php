...
class elFinder
{
    ...
    protected function upload($args)
    {
        ...
        $dosya_boyutu = isset($args['FILES']['upload']['size']) ? array_sum($args['FILES']['upload']['size']) : 0;
        $isFull = isDiskFull($kullanici_id, $conn, $dosya_boyutu);
        if ($isFull) {
            return array('error' => $this->error('The file which you are trying to upload exceed the quota size .'));
            exit();
        }
